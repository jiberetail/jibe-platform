    ...
    <>
      <header
        className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-300 ${
          scrolled ? "border-[#D9D9D9] bg-[#F7F7F7]/95 shadow-[0_8px_30px_rgba(24,24,24,0.06)]" : "border-transparent bg-[#F7F7F7]/90"
        }`}
            ...
            <div ref={productsRef} className="relative">
              ...
              {productsOpen && (
                <div
                  ...
                  className="absolute left-0 top-full mt-2 w-[350px] overflow-hidden rounded-2xl border border-[#D9D9D9] bg-white p-2 shadow-[0_24px_70px_rgba(26,31,37,0.16)]"
                >
                  {products.map((product) => (
                    <Link
                      key={product.href}
                      to={product.href}
                      className="group flex items-start justify-between gap-5 rounded-xl px-4 py-4 transition-colors hover:bg-[#F2F2F2] focus-visible:outline-2 focus-visible:outline-[#0076CE]"
                    >
                      <span>
                        <span className="block text-[14px] font-semibold text-[#2F2F2F]">{product.label}</span>
                        <span className="mt-1 block text-[12px] leading-relaxed text-[#6D6D6D]">{product.description}</span>
                      </span>
            ...
            <div ref={companyRef} className="relative">
              ...
              {companyOpen && (
                <div
                  ...
                  className="absolute left-0 top-full mt-2 w-[330px] overflow-hidden rounded-2xl border border-[#D9D9D9] bg-white p-2 shadow-[0_24px_70px_rgba(26,31,37,0.16)]"
                >
                  {companyPages.map((page) => (
                    <Link
                      key={page.href}
                      to={page.href}
                      className="group flex items-start justify-between gap-5 rounded-xl px-4 py-3.5 transition-colors hover:bg-[#F2F2F2] focus-visible:outline-2 focus-visible:outline-[#0076CE]"
                    >
                      <span>
                        <span className="block text-[14px] font-semibold text-[#2F2F2F]">{page.label}</span>
                        <span className="mt-1 block text-[12px] leading-relaxed text-[#6D6D6D]">{page.description}</span>
                      </span>
                      <ArrowUpRight size={15} className="mt-0.5 shrink-0 text-[#9B9D9F] transition-colors group-hover:text-[#0076CE]" />
                    </Link>
                  ))}
      ...
      {mobileOpen && (
        <div
          ref={mobileDialogRef}
          className="fixed inset-0 z-[100] flex flex-col bg-[#F7F7F7] lg:hidden"
          role="dialog"
          aria-modal="true"
          aria-label="Navigation menu"
        >
          <div className="flex h-[76px] items-center justify-between border-b border-[#D9D9D9] px-5 sm:px-6">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-7 sm:px-6">
            <p className="mb-3 font-mono text-[10px] uppercase tracking-[0.22em] text-[#777777]">Products</p>
            <div className="grid gap-2">
              {products.map((product) => (
                <Link
                  ...
                  to={product.href}
                  onClick={() => setMobileOpen(false)}
                  className="rounded-2xl border border-[#D9D9D9] bg-white p-5"
                >
                  ...
                  <span className="mt-1 block text-[13px] text-[#6D6D6D]">{product.description}</span>
                </Link>
              ))}
            </div>

            <div className="mt-8 border-t border-[#D9D9D9] pt-4">
              <p className="mb-1 font-mono text-[10px] uppercase tracking-[0.22em] text-[#777777]">Company</p>
              {companyPages.map((page) => (
                <Link
                  key={page.href}
                  to={page.href}
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center justify-between border-b border-[#D9D9D9] py-3.5 text-[19px] font-medium text-[#2F2F2F]"
                >
                  {page.label}
            ...
            <div className="mt-6 border-t border-[#D9D9D9] pt-1">
              {sectionLinks.map((link) => (
                <Link
                  key={link.href}
                  to={link.href}
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center justify-between border-b border-[#D9D9D9] py-4 text-[22px] font-medium text-[#2F2F2F]"
                >
                  {link.label}
